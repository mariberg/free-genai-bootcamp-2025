# TODO - # Query to fetch words with sorting - not sure if I want to sort with both
## types of spellings
...
def load(app):
  ...
  def get_words():
    try:
      ...
      sort_by = request.args.get('sort_by', 'latin_spelling')  # Default to sorting by 'kanji'
      order = request.args.get('order', 'asc')  # Default to ascending order

      # Validate sort_by and order
      valid_columns = ['latin_spelling', 'origin_language', 'arabic_spelling', 'english', 'correct_count', 'wrong_count']
      if sort_by not in valid_columns:
        sort_by = 'latin_spelling'
      if order not in ['asc', 'desc']:
        order = 'asc'

      # Query to fetch words with sorting
      cursor.execute(f'''
        SELECT w.id, w.latin_spelling, w.arabic_spelling, w.english,
      ...
      for word in words:
        words_data.append({
          "id": word["id"],
          "latin_spelling": word["latin_spelling"],
          "origin_language": word["origin_language"],
          "arabic_spelling": word["arabic_spelling"],
